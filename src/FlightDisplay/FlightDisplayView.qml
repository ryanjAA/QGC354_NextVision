...
QGCView {
    ...
    QGCViewPanel {
        ...
        ToolStrip {
            ...
            id:                 camToolStrip
            anchors.leftMargin: isInstrumentRight() ? ScreenTools.defaultFontPixelWidth : undefined
            anchors.left:       isInstrumentRight() ? toolStrip.right : undefined
            anchors.rightMargin:isInstrumentRight() ? undefined : ScreenTools.defaultFontPixelWidth
            anchors.right:      isInstrumentRight() ? undefined : _panel.right
            anchors.topMargin:  ScreenTools.toolbarHeight + (_margins * 2)
            anchors.top:        _panel.top
            ...
            title:              qsTr("Cam")
            maxHeight:          (_flightVideo.visible ? _flightVideo.y : parent.height) - toolStrip.y
            buttonVisible:      [ true,true,true,true ]
            buttonEnabled:      [ _guidedController.enablePtc,true,true,true ]
            ...
            property var _actionModel: [
                {
                    title:      _guidedController.startMissionTitle,
                    text:       _guidedController.startMissionMessage,
                    action:     _guidedController.actionPtc,
                    visible:    true
                },
                {
                    title:      _guidedController.continueMissionTitle,
                    text:       _guidedController.continueMissionMessage,
                    action:     _guidedController.actionDayIR,
                    visible:    true
                },
                {
                    title:      _guidedController.obsTitle,
                    text:       _guidedController.continueMissionMessage,
                    action:     _guidedController.actionObs,
                    visible:    true
                },
                {
                    title:      _guidedController.grrTitle,
                    text:       _guidedController.continueMissionMessage,
                    action:     _guidedController.actionGrr,
                    visible:    true
                }
            ]
            ...
            model: [
                {
                    name:       _guidedController.ptcTitle,
                    iconSource: "/qmlimages/PtcIcon",
                    action:     _guidedController.actionPtc
                },
                {
                    name:       _guidedController.dayIRTitle,
                    iconSource: "/qmlimages/DayIR",
                    action:     _guidedController.actionDayIR
                },
                {
                    name:       _guidedController.obsTitle,
                    iconSource: "/qmlimages/ObsIcon",
                    action:     _guidedController.actionObs
                },
                {
                    name:       _guidedController.grrTitle,
                    iconSource: "/qmlimages/GrrIcon",
                    action:     _guidedController.actionGrr
                }
            ]
            ...
            onClicked: {
                ...
                if (action === -1) {
                    ...
                    _guidedController.confirmAction(action)
                    if(action === _guidedController.actionDayIR)
                        joystickManager.cameraManagement.setSysSensorToggleCommand();
                    else if(action === _guidedController.actionPtc)
                        _guidedController.enablePtc = _guidedController.enablePtc?false:true
                    else if(action === _guidedController.actionObs)
                        joystickManager.cameraManagement.setSysModeObsCommand();
                    else if(action === _guidedController.actionGrr)
