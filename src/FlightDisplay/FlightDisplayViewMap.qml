...
FlightMap {
    ...
    MouseArea {
        ...
        onClicked: {
            ...
            /* In case of left Click  - perform point to coordinate if PTC button is clicked */
            if (mouse.button == Qt.LeftButton){
                if(!guidedActionsController.enablePtc)
                {
                    /* enable PTC button again */
                    guidedActionsController.enablePtc = true
                    if(_activeVehicle)
                        joystickManager.cameraManagement.pointToCoordinate(clickCoord.latitude,clickCoord.longitude)
                }
            }else{
                /* In case of Right Click  - enable the right click menu if taked-off */
        ...
        onPressAndHold : {
            /* In case of long press  - enable the long press menu if taked-off */
            var clickCoord = flightMap.toCoordinate(Qt.point(mouse.x, mouse.y),false)
            if (guidedActionsController.guidedUIVisible || (!guidedActionsController.showGotoLocation && !guidedActionsController.showOrbit)) {
                return
            }
